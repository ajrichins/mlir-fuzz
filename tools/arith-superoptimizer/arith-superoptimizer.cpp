...
bool executeAndSaveModule(mlir::ModuleOp module, StringRef outputDirectory)
{
  ...
  {
    std::string hashStr;
    for (int32_t input1 : interestingValues)
    {
      for (int32_t input2 : interestingValues)
      {
    ...
    std::string directory = (outputDirectory + "/hash_a" + std::to_string(numArguments) + hashStr).str();
...
int main(int argc, char **argv)
{
  ...
  // Number of program arguments to be printed.
  static llvm::cl::opt<int> maxNumArgs(
      "max-num-args",
      llvm::cl::desc("Maximum number of program arguments"),
      llvm::cl::init(2));
  ...
  while (auto chooser = guide.makeChooser())
  {
    auto module = createProgram(ctx, availableOps, availableTypes(ctx),
                                ...
                                maxNumOps, maxNumArgs, correctProgramCounter); // replace 1 wiht maxNumArgs Later
    if (!module)
      continue;
